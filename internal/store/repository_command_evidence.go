...
func CanonicalRepositoryCommandEvidenceRequest(value RepositoryCommandEvidenceRequest) ([]byte, string, error) {
	if (value.Purpose != RepositoryCommandPurposePrebuildVerification && value.Purpose != RepositoryCommandPurposePostbuildCandidate) || value.Ref.Validate() != nil || value.TicketVersion == 0 || value.LeaderEpoch == 0 || value.RunnerEpoch == 0 || value.ProviderResult.AttemptID <= 0 || value.ProviderResult.Ref != value.Ref || value.ProviderResult.Attempt <= 0 || (value.Purpose == RepositoryCommandPurposePrebuildVerification && (value.ProviderResult.Phase != domain.PhaseVerification || value.CheckpointID != "")) || (value.Purpose == RepositoryCommandPurposePostbuildCandidate && (value.ProviderResult.Phase != domain.PhaseBuild || !validOID(value.CheckpointID))) || !validDigest(value.VerificationIntentDigest) || !validDigest(value.ProofDigest) || !validClaimDigest(value.ConfigCommandDigest) || !validStorePath(value.Worktree) || value.WorktreeIdentity == "" || !validOID(value.BaseSHA) || !validClaimDigest(value.PolicyDigest) || !validClaimDigest(value.SpecDigest) || !validStorePath(value.ExecutablePath) || !validClaimDigest(value.ExecutableDigest) {
