...
func (s *Store) RecordMergeIntent(ctx context.Context, intent domain.MergeIntent) error {
	...
	return s.write(ctx, func(conn *sql.Conn) error {
		...
		result, err := conn.ExecContext(ctx, `INSERT INTO merge_intents(semantic_key, channel, project_id, ticket_id, request_digest, ticket_version, leader_epoch, runner_epoch, claim_epoch, repository_host, repository_owner, repository_name, pull_request_number, head_oid, base_ref, original_base_oid, protection_rule_id, strict_status_checks, method, created_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			ON CONFLICT(semantic_key) DO NOTHING`,
			intent.SemanticKey, intent.Ref.Channel, intent.Ref.Project, intent.Ref.Ticket, intent.RequestDigest, intent.TicketVersion, intent.LeaderEpoch, intent.RunnerEpoch, intent.ClaimEpoch, intent.RepositoryHost, intent.RepositoryOwner, intent.RepositoryName, intent.PullRequestNumber, intent.HeadOID, intent.BaseRef, intent.OriginalBaseOID, intent.ProtectionRuleID, boolInt(intent.StrictStatusChecks), intent.Method, time.Now().UTC().Format(time.RFC3339Nano))
		if err != nil {
			return err
		}
		...
		var existingDigest, existingHead, existingRef, existingBase, existingMethod string
		if err := conn.QueryRowContext(ctx, `SELECT request_digest, head_oid, base_ref, original_base_oid, method FROM merge_intents WHERE semantic_key=?`, intent.SemanticKey).Scan(&existingDigest, &existingHead, &existingRef, &existingBase, &existingMethod); err != nil {
			return err
		}
		if existingDigest != intent.RequestDigest || existingHead != intent.HeadOID || existingRef != intent.BaseRef || existingBase != intent.OriginalBaseOID || existingMethod != intent.Method {
			return ErrEffectKey
		}
		return nil
	})
...
func (s *Store) MergeIntent(ctx context.Context, semanticKey string) (domain.MergeIntent, bool, error) {
	...
	var strict int
	err := s.db.QueryRowContext(ctx, `SELECT channel, project_id, ticket_id, request_digest, ticket_version, leader_epoch, runner_epoch, claim_epoch, repository_host, repository_owner, repository_name, pull_request_number, head_oid, base_ref, original_base_oid, protection_rule_id, strict_status_checks, method FROM merge_intents WHERE semantic_key=?`, semanticKey).Scan(
		&intent.Ref.Channel, &intent.Ref.Project, &intent.Ref.Ticket, &intent.RequestDigest, &intent.TicketVersion, &intent.LeaderEpoch, &intent.RunnerEpoch, &intent.ClaimEpoch, &intent.RepositoryHost, &intent.RepositoryOwner, &intent.RepositoryName, &intent.PullRequestNumber, &intent.HeadOID, &intent.BaseRef, &intent.OriginalBaseOID, &intent.ProtectionRuleID, &strict, &intent.Method)
	if err == sql.ErrNoRows {
		return domain.MergeIntent{}, false, nil
	}
	intent.SemanticKey = semanticKey
	intent.StrictStatusChecks = strict == 1
