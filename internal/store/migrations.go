...
// v41 records authenticated CI observations and the append-only evidence
// needed to authorize a bounded correction. Every row carries the exact
// candidate/publication identity it observed; no polling or repair execution
// belongs in this migration. The forthcoming Store write API must recompute
// required_set_digest and observation_digest from canonical checks before
// inserting these rows; this schema reducer is necessary, but not sufficient,
// runtime authority.
var migrationV41 = []string{
	`ALTER TABLE publication_evidence ADD COLUMN github_open INTEGER NOT NULL DEFAULT 1 CHECK(github_open=1)`,
	`CREATE UNIQUE INDEX candidate_snapshot_generation_head ON candidate_snapshots(channel,project_id,ticket_id,generation,head_sha)`,
	`CREATE UNIQUE INDEX candidate_snapshot_generation_head_tree ON candidate_snapshots(channel,project_id,ticket_id,generation,head_sha,tree_sha)`,
	`CREATE UNIQUE INDEX publication_evidence_candidate_witness ON publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,witness_digest)`,
	`CREATE UNIQUE INDEX publication_evidence_candidate_witness_tree ON publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest)`,
	`CREATE UNIQUE INDEX publication_evidence_ci_authority ON publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest,github_host,github_owner,github_name,github_pr_number,github_head_owner,github_head_repository,github_head_ref,github_head_oid,github_base_ref,github_base_oid,github_open,github_draft,github_factory_owned)`,
	`CREATE UNIQUE INDEX publication_evidence_repair_authority ON publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest,github_host,github_owner,github_name,github_pr_number,remote_branch_ref,remote_branch_oid,github_base_ref,remote_base_oid)`,
	`CREATE UNIQUE INDEX provider_attempts_builder_identity ON provider_attempts(channel,project_id,ticket_id,phase,role,attempt,id)`,
	`CREATE UNIQUE INDEX provider_attempt_results_builder_identity ON provider_attempt_results(channel,project_id,ticket_id,phase,role,attempt,provider_attempt_id)`,
	`CREATE UNIQUE INDEX candidate_result_bindings_completion_identity ON candidate_result_bindings(channel,project_id,ticket_id,generation,binding_ticket_version,leader_epoch,runner_epoch,provider_attempt_id,provider_attempt)`,
	`CREATE UNIQUE INDEX events_identity ON events(channel,project_id,ticket_id,ticket_version,id,created_at)`,
	`CREATE UNIQUE INDEX events_transition_identity ON events(channel,project_id,ticket_id,ticket_version,id,created_at,from_state,to_state,trigger)`,
	`CREATE UNIQUE INDEX ticket_budget_uses_correction_identity ON ticket_budget_uses(channel,project_id,ticket_id,kind,request_id,ticket_version,leader_epoch,runner_epoch)`,
	`CREATE TABLE ci_observations (
		observation_id INTEGER PRIMARY KEY AUTOINCREMENT,
		channel TEXT NOT NULL CHECK(channel IN ('stable','dev')), project_id TEXT NOT NULL, ticket_id TEXT NOT NULL,
		candidate_generation INTEGER NOT NULL CHECK(candidate_generation > 0), candidate_head_sha TEXT NOT NULL CHECK(length(candidate_head_sha) BETWEEN 1 AND 128), candidate_tree_sha TEXT NOT NULL CHECK(length(candidate_tree_sha) BETWEEN 1 AND 128),
		publication_witness_digest TEXT NOT NULL CHECK(length(publication_witness_digest)=71),
		pr_host TEXT NOT NULL CHECK(length(pr_host) BETWEEN 1 AND 255), pr_owner TEXT NOT NULL CHECK(length(pr_owner) BETWEEN 1 AND 100), pr_repo TEXT NOT NULL CHECK(length(pr_repo) BETWEEN 1 AND 200), pr_number INTEGER NOT NULL CHECK(pr_number > 0),
		pr_head_owner TEXT NOT NULL CHECK(length(pr_head_owner) BETWEEN 1 AND 100), pr_head_repo TEXT NOT NULL CHECK(length(pr_head_repo) BETWEEN 1 AND 200), pr_head_ref TEXT NOT NULL CHECK(length(pr_head_ref) BETWEEN 1 AND 255), pr_head_oid TEXT NOT NULL CHECK(length(pr_head_oid) BETWEEN 1 AND 128),
		pr_base_ref TEXT NOT NULL CHECK(length(pr_base_ref) BETWEEN 1 AND 255), pr_base_oid TEXT NOT NULL CHECK(length(pr_base_oid) BETWEEN 1 AND 128),
		pr_factory_owned INTEGER NOT NULL CHECK(pr_factory_owned=1), pr_open INTEGER NOT NULL CHECK(pr_open=1), pr_draft INTEGER NOT NULL CHECK(pr_draft=1),
		observed_ticket_version INTEGER NOT NULL CHECK(observed_ticket_version > 0), observed_leader_epoch INTEGER NOT NULL CHECK(observed_leader_epoch > 0), observed_runner_epoch INTEGER NOT NULL CHECK(observed_runner_epoch > 0), observed_at TEXT NOT NULL CHECK(length(observed_at) BETWEEN 1 AND 128),
		required_set_digest TEXT NOT NULL CHECK(length(required_set_digest)=64), required_check_count INTEGER NOT NULL CHECK(required_check_count > 0),
		classification TEXT NOT NULL CHECK(classification IN ('pending','green','red')),
		diagnostic_digest TEXT NOT NULL DEFAULT '' CHECK(diagnostic_digest='' OR length(diagnostic_digest) IN (64,71)), diagnostic_text TEXT NOT NULL DEFAULT '' CHECK(length(diagnostic_text) <= 16384), diagnostic_json TEXT NOT NULL DEFAULT '' CHECK(length(diagnostic_json) <= 65536 AND (diagnostic_json='' OR json_valid(diagnostic_json)=1)),
		observation_digest TEXT NOT NULL CHECK(length(observation_digest)=71),
		UNIQUE(observation_digest), UNIQUE(observation_id,observation_digest),
		FOREIGN KEY(channel,project_id,ticket_id) REFERENCES tickets(channel,project_id,id),
		...
		FOREIGN KEY(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,publication_witness_digest,pr_host,pr_owner,pr_repo,pr_number,pr_head_owner,pr_head_repo,pr_head_ref,pr_head_oid,pr_base_ref,pr_base_oid,pr_open,pr_draft,pr_factory_owned) REFERENCES publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest,github_host,github_owner,github_name,github_pr_number,github_head_owner,github_head_repository,github_head_ref,github_head_oid,github_base_ref,github_base_oid,github_open,github_draft,github_factory_owned)
	)`,
	`CREATE UNIQUE INDEX ci_observations_candidate_digest ON ci_observations(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,observation_digest)`,
	`CREATE UNIQUE INDEX ci_observations_candidate_tree_digest ON ci_observations(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,observation_digest)`,
	`CREATE UNIQUE INDEX ci_observations_candidate_tree_classification_digest ON ci_observations(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,classification,observation_digest)`,
	`CREATE UNIQUE INDEX ci_observations_transition_authority ON ci_observations(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,classification,observation_digest,observed_ticket_version,observed_leader_epoch,observed_runner_epoch)`,
	`CREATE UNIQUE INDEX ci_observations_digest ON ci_observations(observation_digest)`,
	`CREATE TABLE ci_observation_checks (
		observation_id INTEGER NOT NULL, observation_digest TEXT NOT NULL,
		canonical_name TEXT NOT NULL CHECK(length(canonical_name) BETWEEN 1 AND 255), external_id TEXT NOT NULL CHECK(length(external_id) BETWEEN 1 AND 255),
		normalized_state TEXT NOT NULL CHECK(normalized_state IN ('pending','success','failure','cancelled','skipped','neutral')),
		failing_diagnostic_digest TEXT NOT NULL DEFAULT '' CHECK(failing_diagnostic_digest='' OR length(failing_diagnostic_digest) IN (64,71)), failing_diagnostic_text TEXT NOT NULL DEFAULT '' CHECK(length(failing_diagnostic_text) <= 16384),
		PRIMARY KEY(observation_id,canonical_name,external_id), UNIQUE(observation_digest,canonical_name,external_id),
		FOREIGN KEY(observation_id,observation_digest) REFERENCES ci_observations(observation_id,observation_digest)
	)`,
	`CREATE TABLE ci_transition_evidence (
		channel TEXT NOT NULL CHECK(channel IN ('stable','dev')), project_id TEXT NOT NULL, ticket_id TEXT NOT NULL,
		candidate_generation INTEGER NOT NULL CHECK(candidate_generation > 0), candidate_head_sha TEXT NOT NULL CHECK(length(candidate_head_sha) BETWEEN 1 AND 128), candidate_tree_sha TEXT NOT NULL CHECK(length(candidate_tree_sha) BETWEEN 1 AND 128),
		ticket_version INTEGER NOT NULL CHECK(ticket_version > 0), event_id INTEGER NOT NULL CHECK(event_id > 0), event_created_at TEXT NOT NULL CHECK(length(event_created_at) BETWEEN 1 AND 128),
		observation_classification TEXT NOT NULL CHECK(observation_classification IN ('pending','green','red')), observation_digest TEXT NOT NULL CHECK(length(observation_digest)=71), observation_ticket_version INTEGER NOT NULL CHECK(observation_ticket_version > 0), observation_leader_epoch INTEGER NOT NULL CHECK(observation_leader_epoch > 0), observation_runner_epoch INTEGER NOT NULL CHECK(observation_runner_epoch > 0), prior_publication_witness_digest TEXT NOT NULL CHECK(length(prior_publication_witness_digest)=71),
		prior_state TEXT NOT NULL CHECK(prior_state='waiting_ci'), resulting_state TEXT NOT NULL CHECK(resulting_state IN ('waiting_ci','reviewing','building','paused')),
		resulting_trigger TEXT NOT NULL CHECK((observation_classification='pending' AND resulting_trigger='checks_pending') OR (observation_classification='green' AND resulting_trigger='checks_green') OR (observation_classification='red' AND resulting_trigger='checks_red')), transition_digest TEXT NOT NULL CHECK(length(transition_digest)=71), created_at TEXT NOT NULL CHECK(length(created_at) BETWEEN 1 AND 128),
		CHECK(ticket_version=observation_ticket_version+1), CHECK((observation_classification='pending' AND resulting_state='waiting_ci') OR (observation_classification='green' AND resulting_state='reviewing') OR (observation_classification='red' AND resulting_state IN ('building','paused'))),
		PRIMARY KEY(channel,project_id,ticket_id,ticket_version), UNIQUE(transition_digest),
		FOREIGN KEY(channel,project_id,ticket_id) REFERENCES tickets(channel,project_id,id),
		FOREIGN KEY(channel,project_id,ticket_id,ticket_version,event_id,event_created_at,prior_state,resulting_state,resulting_trigger) REFERENCES events(channel,project_id,ticket_id,ticket_version,id,created_at,from_state,to_state,trigger),
		FOREIGN KEY(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,observation_classification,observation_digest,observation_ticket_version,observation_leader_epoch,observation_runner_epoch) REFERENCES ci_observations(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,classification,observation_digest,observed_ticket_version,observed_leader_epoch,observed_runner_epoch),
		FOREIGN KEY(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,prior_publication_witness_digest) REFERENCES publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest)
	)`,
	`CREATE UNIQUE INDEX ci_transition_evidence_red_identity ON ci_transition_evidence(channel,project_id,ticket_id,ticket_version,observation_classification,transition_digest)`,
	`CREATE UNIQUE INDEX ci_transition_evidence_authority ON ci_transition_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,observation_classification,observation_digest,prior_publication_witness_digest,observation_ticket_version,observation_leader_epoch,observation_runner_epoch,ticket_version,transition_digest)`,
	`CREATE TABLE candidate_repair_bindings (
		channel TEXT NOT NULL CHECK(channel IN ('stable','dev')), project_id TEXT NOT NULL, ticket_id TEXT NOT NULL, target_generation INTEGER NOT NULL CHECK(target_generation > 0),
		predecessor_generation INTEGER NOT NULL CHECK(predecessor_generation > 0), predecessor_head_sha TEXT NOT NULL CHECK(length(predecessor_head_sha) BETWEEN 1 AND 128), predecessor_tree_sha TEXT NOT NULL CHECK(length(predecessor_tree_sha) BETWEEN 1 AND 128), predecessor_publication_witness_digest TEXT NOT NULL CHECK(length(predecessor_publication_witness_digest)=71),
		pr_host TEXT NOT NULL CHECK(length(pr_host) BETWEEN 1 AND 255), pr_owner TEXT NOT NULL CHECK(length(pr_owner) BETWEEN 1 AND 100), pr_repo TEXT NOT NULL CHECK(length(pr_repo) BETWEEN 1 AND 200), pr_number INTEGER NOT NULL CHECK(pr_number > 0),
		branch_ref TEXT NOT NULL CHECK(length(branch_ref) BETWEEN 1 AND 255), remote_head_oid TEXT NOT NULL CHECK(length(remote_head_oid) BETWEEN 1 AND 128), base_ref TEXT NOT NULL CHECK(length(base_ref) BETWEEN 1 AND 255), remote_base_oid TEXT NOT NULL CHECK(length(remote_base_oid) BETWEEN 1 AND 128),
		red_observation_digest TEXT NOT NULL CHECK(length(red_observation_digest)=71), red_observation_classification TEXT NOT NULL CHECK(red_observation_classification='red'), red_transition_ticket_version INTEGER NOT NULL CHECK(red_transition_ticket_version > 0), red_transition_digest TEXT NOT NULL CHECK(length(red_transition_digest)=71), correction_budget_kind TEXT NOT NULL CHECK(correction_budget_kind='correction'), correction_budget_request_id TEXT NOT NULL CHECK(length(correction_budget_request_id) BETWEEN 1 AND 300),
		consumed_ticket_version INTEGER NOT NULL CHECK(consumed_ticket_version > 0), consumed_leader_epoch INTEGER NOT NULL CHECK(consumed_leader_epoch > 0), consumed_runner_epoch INTEGER NOT NULL CHECK(consumed_runner_epoch > 0), repair_context_digest TEXT NOT NULL CHECK(length(repair_context_digest)=71), created_at TEXT NOT NULL CHECK(length(created_at) BETWEEN 1 AND 128), CHECK(red_transition_ticket_version=consumed_ticket_version+1),
		PRIMARY KEY(channel,project_id,ticket_id,target_generation), CHECK(target_generation=predecessor_generation+1),
		FOREIGN KEY(channel,project_id,ticket_id) REFERENCES tickets(channel,project_id,id), FOREIGN KEY(channel,project_id,ticket_id,predecessor_generation,predecessor_head_sha,predecessor_tree_sha) REFERENCES candidate_snapshots(channel,project_id,ticket_id,generation,head_sha,tree_sha),
		FOREIGN KEY(channel,project_id,ticket_id,predecessor_generation,predecessor_head_sha,predecessor_tree_sha,red_observation_classification,red_observation_digest) REFERENCES ci_observations(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,classification,observation_digest),
		FOREIGN KEY(channel,project_id,ticket_id,predecessor_generation,predecessor_head_sha,predecessor_tree_sha,predecessor_publication_witness_digest,pr_host,pr_owner,pr_repo,pr_number,branch_ref,remote_head_oid,base_ref,remote_base_oid) REFERENCES publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest,github_host,github_owner,github_name,github_pr_number,remote_branch_ref,remote_branch_oid,github_base_ref,remote_base_oid),
		FOREIGN KEY(channel,project_id,ticket_id,predecessor_generation,predecessor_head_sha,predecessor_tree_sha,red_observation_classification,red_observation_digest,predecessor_publication_witness_digest,consumed_ticket_version,consumed_leader_epoch,consumed_runner_epoch,red_transition_ticket_version,red_transition_digest) REFERENCES ci_transition_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,observation_classification,observation_digest,prior_publication_witness_digest,observation_ticket_version,observation_leader_epoch,observation_runner_epoch,ticket_version,transition_digest),
	...
	`CREATE TABLE candidate_repair_completions (
		channel TEXT NOT NULL CHECK(channel IN ('stable','dev')), project_id TEXT NOT NULL, ticket_id TEXT NOT NULL, target_generation INTEGER NOT NULL CHECK(target_generation > 0),
		builder_result_attempt_id INTEGER NOT NULL CHECK(builder_result_attempt_id > 0), builder_result_attempt INTEGER NOT NULL CHECK(builder_result_attempt > 0), builder_result_phase TEXT NOT NULL CHECK(builder_result_phase='build'), builder_result_role TEXT NOT NULL CHECK(builder_result_role='builder'), builder_binding_ticket_version INTEGER NOT NULL CHECK(builder_binding_ticket_version > 0), builder_binding_leader_epoch INTEGER NOT NULL CHECK(builder_binding_leader_epoch > 0), builder_binding_runner_epoch INTEGER NOT NULL CHECK(builder_binding_runner_epoch > 0), final_candidate_head_sha TEXT NOT NULL CHECK(length(final_candidate_head_sha) BETWEEN 1 AND 128), final_candidate_tree_sha TEXT NOT NULL CHECK(length(final_candidate_tree_sha) BETWEEN 1 AND 128),
		completion_digest TEXT NOT NULL CHECK(length(completion_digest)=71), completed_at TEXT NOT NULL CHECK(length(completed_at) BETWEEN 1 AND 128),
		PRIMARY KEY(channel,project_id,ticket_id,target_generation), UNIQUE(completion_digest),
		FOREIGN KEY(channel,project_id,ticket_id,target_generation) REFERENCES candidate_repair_bindings(channel,project_id,ticket_id,target_generation),
		...
		FOREIGN KEY(channel,project_id,ticket_id,target_generation,builder_binding_ticket_version,builder_binding_leader_epoch,builder_binding_runner_epoch,builder_result_attempt_id,builder_result_attempt) REFERENCES candidate_result_bindings(channel,project_id,ticket_id,generation,binding_ticket_version,leader_epoch,runner_epoch,provider_attempt_id,provider_attempt),
		FOREIGN KEY(channel,project_id,ticket_id,target_generation,final_candidate_head_sha,final_candidate_tree_sha) REFERENCES candidate_snapshots(channel,project_id,ticket_id,generation,head_sha,tree_sha)
	)`,
	`CREATE INDEX ci_observations_latest ON ci_observations(channel,project_id,ticket_id,candidate_generation DESC,observed_at DESC,observation_id DESC)`,
	`CREATE INDEX ci_observation_checks_observation ON ci_observation_checks(observation_id,canonical_name,external_id)`,
	`CREATE INDEX ci_transition_evidence_chain ON ci_transition_evidence(channel,project_id,ticket_id,ticket_version,event_id)`,
	`CREATE UNIQUE INDEX ci_transition_evidence_digest ON ci_transition_evidence(transition_digest)`,
	`CREATE INDEX candidate_repair_bindings_predecessor ON candidate_repair_bindings(channel,project_id,ticket_id,predecessor_generation,predecessor_head_sha)`,
	`CREATE INDEX candidate_repair_bindings_predecessor_lineage ON candidate_repair_bindings(channel,project_id,ticket_id,predecessor_generation,predecessor_head_sha,predecessor_tree_sha)`,
	`CREATE INDEX candidate_repair_bindings_target ON candidate_repair_bindings(channel,project_id,ticket_id,target_generation)`,
	`CREATE UNIQUE INDEX candidate_repair_bindings_context_digest ON candidate_repair_bindings(repair_context_digest)`,
	`CREATE UNIQUE INDEX candidate_repair_completions_digest ON candidate_repair_completions(completion_digest)`,
	`CREATE TRIGGER ci_observations_immutable_update BEFORE UPDATE ON ci_observations BEGIN SELECT RAISE(ABORT,'ci observation is immutable'); END`,
	`CREATE TRIGGER ci_observations_immutable_delete BEFORE DELETE ON ci_observations BEGIN SELECT RAISE(ABORT,'ci observation is append-only'); END`,
	`CREATE TRIGGER ci_observation_checks_immutable_update BEFORE UPDATE ON ci_observation_checks BEGIN SELECT RAISE(ABORT,'ci observation check is immutable'); END`,
	`CREATE TRIGGER ci_observation_checks_immutable_delete BEFORE DELETE ON ci_observation_checks BEGIN SELECT RAISE(ABORT,'ci observation checks are append-only'); END`,
	`CREATE TRIGGER ci_transition_evidence_immutable_update BEFORE UPDATE ON ci_transition_evidence BEGIN SELECT RAISE(ABORT,'ci transition evidence is immutable'); END`,
	`CREATE TRIGGER ci_transition_evidence_immutable_delete BEFORE DELETE ON ci_transition_evidence BEGIN SELECT RAISE(ABORT,'ci transition evidence is append-only'); END`,
	...
	`CREATE TRIGGER candidate_repair_bindings_immutable_update BEFORE UPDATE ON candidate_repair_bindings BEGIN SELECT RAISE(ABORT,'candidate repair binding is immutable'); END`,
	`CREATE TRIGGER candidate_repair_bindings_immutable_delete BEFORE DELETE ON candidate_repair_bindings BEGIN SELECT RAISE(ABORT,'candidate repair binding is append-only'); END`,
	`CREATE TRIGGER candidate_repair_completions_immutable_update BEFORE UPDATE ON candidate_repair_completions BEGIN SELECT RAISE(ABORT,'candidate repair completion is immutable'); END`,
	`CREATE TRIGGER candidate_repair_completions_immutable_delete BEFORE DELETE ON candidate_repair_completions BEGIN SELECT RAISE(ABORT,'candidate repair completion is append-only'); END`,
...
// v48 persists the read-only manual merge observation together with the
// waiting_manual_merge -> reconciling boundary. The row is append-only: a
// restart can authenticate the exact PR/candidate/base witness without ever
// retrying a GitHub merge mutation.
var migrationV48 = []string{
	`CREATE UNIQUE INDEX publication_evidence_manual_merge_authority ON publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest,github_host,github_owner,github_name,github_pr_number,github_head_owner,github_head_repository,github_head_ref,github_head_oid,github_base_ref,github_base_oid,github_factory_owned)`,
	`CREATE TABLE manual_merge_observations (
		observation_id INTEGER PRIMARY KEY AUTOINCREMENT,
		channel TEXT NOT NULL CHECK(channel IN ('stable','dev')), project_id TEXT NOT NULL, ticket_id TEXT NOT NULL,
		current_ticket_version INTEGER NOT NULL CHECK(current_ticket_version > 0), current_leader_epoch INTEGER NOT NULL CHECK(current_leader_epoch > 0), current_runner_epoch INTEGER NOT NULL CHECK(current_runner_epoch > 0),
		candidate_generation INTEGER NOT NULL CHECK(candidate_generation > 0), candidate_head_sha TEXT NOT NULL CHECK(length(candidate_head_sha) BETWEEN 1 AND 128), candidate_base_sha TEXT NOT NULL CHECK(length(candidate_base_sha) BETWEEN 1 AND 128), candidate_tree_sha TEXT NOT NULL CHECK(length(candidate_tree_sha) BETWEEN 1 AND 128), publication_witness_digest TEXT NOT NULL CHECK(length(publication_witness_digest)=71),
		publication_host TEXT NOT NULL CHECK(length(publication_host) BETWEEN 1 AND 128), publication_owner TEXT NOT NULL CHECK(length(publication_owner) BETWEEN 1 AND 128), publication_name TEXT NOT NULL CHECK(length(publication_name) BETWEEN 1 AND 128), publication_pr_number INTEGER NOT NULL CHECK(publication_pr_number > 0), publication_head_owner TEXT NOT NULL CHECK(length(publication_head_owner) BETWEEN 1 AND 128), publication_head_repository TEXT NOT NULL CHECK(length(publication_head_repository) BETWEEN 1 AND 128), publication_head_ref TEXT NOT NULL CHECK(length(publication_head_ref) BETWEEN 1 AND 300), publication_head_oid TEXT NOT NULL CHECK(length(publication_head_oid) BETWEEN 1 AND 128), publication_base_ref TEXT NOT NULL CHECK(length(publication_base_ref) BETWEEN 1 AND 255), publication_base_oid TEXT NOT NULL CHECK(length(publication_base_oid) BETWEEN 1 AND 128), publication_factory_owned INTEGER NOT NULL CHECK(publication_factory_owned IN (0,1)),
		observed_host TEXT NOT NULL CHECK(length(observed_host) BETWEEN 1 AND 128), observed_owner TEXT NOT NULL CHECK(length(observed_owner) BETWEEN 1 AND 128), observed_name TEXT NOT NULL CHECK(length(observed_name) BETWEEN 1 AND 128), observed_pr_number INTEGER NOT NULL CHECK(observed_pr_number > 0), observed_head_owner TEXT NOT NULL CHECK(length(observed_head_owner) BETWEEN 1 AND 128), observed_head_repository TEXT NOT NULL CHECK(length(observed_head_repository) BETWEEN 1 AND 128), observed_head_ref TEXT NOT NULL CHECK(length(observed_head_ref) BETWEEN 1 AND 300), observed_head_oid TEXT NOT NULL CHECK(length(observed_head_oid) BETWEEN 1 AND 128), observed_base_ref TEXT NOT NULL CHECK(length(observed_base_ref) BETWEEN 1 AND 255), observed_base_oid TEXT NOT NULL CHECK(length(observed_base_oid) BETWEEN 1 AND 128), observed_factory_owned INTEGER NOT NULL CHECK(observed_factory_owned IN (0,1)),
		merge_commit TEXT NOT NULL CHECK(length(merge_commit) BETWEEN 1 AND 128), observed_protected_base TEXT NOT NULL CHECK(length(observed_protected_base) BETWEEN 1 AND 128), observation_digest TEXT NOT NULL CHECK(length(observation_digest)=71), created_at TEXT NOT NULL CHECK(length(created_at) BETWEEN 1 AND 128),
		UNIQUE(channel,project_id,ticket_id,candidate_generation,candidate_head_sha), UNIQUE(observation_digest),
		FOREIGN KEY(channel,project_id,ticket_id) REFERENCES tickets(channel,project_id,id),
		FOREIGN KEY(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,publication_witness_digest,publication_host,publication_owner,publication_name,publication_pr_number,publication_head_owner,publication_head_repository,publication_head_ref,publication_head_oid,publication_base_ref,publication_base_oid,publication_factory_owned) REFERENCES publication_evidence(channel,project_id,ticket_id,candidate_generation,candidate_head_sha,candidate_tree_sha,witness_digest,github_host,github_owner,github_name,github_pr_number,github_head_owner,github_head_repository,github_head_ref,github_head_oid,github_base_ref,github_base_oid,github_factory_owned)
	)`,
	`CREATE INDEX manual_merge_observations_ticket ON manual_merge_observations(channel,project_id,ticket_id,observation_id)`,
	`CREATE UNIQUE INDEX manual_merge_observations_digest ON manual_merge_observations(observation_digest)`,
