...
func ExistingGitHubConfigDirectory(home, explicit, xdg string) (string, error) {
	...
	if err != nil || info.Mode()&os.ModeSymlink != 0 || !info.IsDir() || info.Mode().Perm()&0o022 != 0 {
		return "", errors.New("GitHub configuration directory must be a real owner-controlled directory")
	}
	stat, ok := info.Sys().(*syscall.Stat_t)
	if !ok || int(stat.Uid) != os.Getuid() && stat.Uid != 0 {
		return "", errors.New("GitHub configuration directory ownership is unsafe")
	}
	canonical, err := filepath.EvalSymlinks(path)
	if err != nil || !safeGitHubConfigPath(canonical) {
		return "", errors.New("GitHub configuration directory could not be authenticated")
	}
	current, err := os.Lstat(canonical)
	if err != nil || !os.SameFile(info, current) {
		return "", errors.New("GitHub configuration directory changed during validation")
	}
