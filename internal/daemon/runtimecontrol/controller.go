...
func New(database *store.Store, runtime *workflowruntime.ControlBundle, observer MergeObserver) (*Controller, error) {
	if database == nil || runtime == nil || !runtime.Valid() {
		return nil, errors.New("store and ticket runtime are required")
	}
	return &Controller{store: database, runtime: runtime, observer: observer, tickets: make(map[domain.TicketRef]*ticketControl)}, nil
}
