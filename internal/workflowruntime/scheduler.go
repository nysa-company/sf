...
func (s Scheduler) activeState(state domain.State) bool {
	...
	return state == domain.StatePlanning || state == domain.StateVerifying || state == domain.StateBuilding || state == domain.StatePublishing || state == domain.StateReviewing || state == domain.StateWaitingManualMerge || state == domain.StateMerging
}
