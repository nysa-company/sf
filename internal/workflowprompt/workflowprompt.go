...
func renderVerification(input VerificationInput) ([]byte, error) {
	...
	if input.Amendment != nil {
		...
		amendmentInstruction = "This is a fresh independent review of a Builder verification amendment. Reject it by returning the existing proof unchanged; accept it only by returning the exact proposed proof digest after writing and validating the replacement proof. The Store-authenticated amendment command is already bound to OUTPUT_BINDING.command; any other command is malformed. Independently check the alleged contradiction against the original acceptance and actual assertions; a Builder claim or failing implementation is not evidence that a test is wrong. Reject a proposal that removes acceptance coverage, skips tests, broadens expected outcomes, or changes the frozen command merely to obtain green. Preserve every original acceptance criterion when correcting a genuinely inconsistent assertion. Do not implement product behavior."
	}
...
Before sealing the proof, check that its assertions are jointly satisfiable by behavior meeting the accepted plan. In particular, an exact expected public diagnostic must not also be forbidden by a broad redaction pattern. Use unique synthetic secret markers for redaction assertions; do not forbid ordinary words that the required public message contains. An expected pre-build failure does not establish that the tests themselves are correct. Preserve all acceptance coverage and do not skip assertions to hide a contradiction.
The controller owns workflow states, transitions, effects, permissions, and merge policy; your output must not select any of them.
` + amendmentInstruction + `
TICKET=` + ticket + `
PLAN=` + plan + `
...
Check the independent assertions for a concrete contradiction before claiming implementation completion. Distinguish an implementation defect from two assertions that cannot both hold for the same accepted behavior. For an alleged contradiction, identify the exact file and conflicting assertions in the bounded amendment reason, explain the original acceptance they protect, and propose only the smallest correction preserving that acceptance. Do not edit the protected files yourself, invent a passing command result, or treat test failure alone as amendment justification; a fresh independent Reviewer decides.
...
PATH is intentionally limited to /usr/bin:/bin. Optional developer tools such as rg may be absent; command not found is not a sandbox denial. Use available read-only file tools or ordinary /bin/cat and /usr/bin/sed commands on the supplied candidate and verification paths. Do not request permission overrides, escalation, writes, or network access. If a read is genuinely denied, report the specific failed read and observed error; do not infer that all inspection is unavailable from an unrelated tool failure. Emit the final decision only after inspection is complete or a concrete blocker is established.
The reviewer JSON schema applies only to your final response, not to tool calls. You may call read-only inspection tools on their required tool channel before producing that final response. Do not emit a provisional reviewer decision before attempting inspection, and do not treat the final-response format as a prohibition on tool use.
